...
func filter(filepathIn, filepathOut string, threads int) {
	...
	if threads == 1 {
		...
		ChanSlice := make([]chan [][]uint8, threads)
		for i := 0; i<threads; i++ {
			ChanSlice[i] = make(chan [][]uint8)
		}
		for i := 0; i < threads; i++ {

			go worker(int(float32(height)*(float32(i)/float32(threads))),
				int(float32(height)*(float32(i+1)/float32(threads))),
				0,
				width,
				immutableData, ChanSlice[i])
		}
...
func worker(startY, endY, startX, endX int, data func(y, x int) uint8, out chan<- [][]uint8) {
	work := medianFilter(startY, endY, startX, endX, data)
	out <- work
}
